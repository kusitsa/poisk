...
st.markdown("""
    ...
    .stButton > button {
        ...
        background-color: #ffdb4d !important; color: black !important; font-weight: bold !important; font-size: 24px;
    }
    .alice-card { 
...
def get_ai_res(prompt_text):
    try:
        # Используем Gemini (нам нужен ключ GEMINI_API_KEY в Secrets)
        genai.configure(api_key=st.secrets["GEMINI_API_KEY"])
        model = genai.GenerativeModel('gemini-1.5-flash')
        response = model.generate_content(prompt_text)
        return response.text
    except Exception as e:
        return f"Ошибка ИИ: {e}. Проверьте GEMINI_API_KEY в Secrets."
...
with st.sidebar:
    ...
    for h in reversed(st.session_state.history[-5:]):
        if st.button(f"🔍 {h}", key=f"h_{h}"):
            st.session_state.last_q = ""; st.session_state.main_search = h; st.rerun()
    ...
    if st.session_state.get('last_q') != q:
        with st.spinner(" "):
            try:
                ...
                prompt = f"Ты Кусица, эрудированный ассистент. Ответь подробно на вопрос: {q}, используя данные: {links[:5]}"
