...
from duckduckgo_search import DDGS # БЕСПЛАТНЫЙ ПОИСК
...
keys_to_init = ['links', 'images', 'videos', 'ai_history', 'last_q', 'view_img_idx', 'chat_extra']
...
def get_header_data():
    try:
        ...
        return now.strftime("%d.%m"), now.strftime("%H:%M"), w, round(c['rates']['RUB'], 1), round(c['rates']['RUB'] / (c['rates']['EUR'] / c['rates']['USD']), 1)
    except: return "??.??", "??:??", "?", "??", "??"

def perform_search(q):
    if not q: return
    if state.last_q == q: return
    ...
    with st.spinner("КУСИЦА ищет в сети..."):
        try:
            with DDGS() as ddgs:
                # 1. Текст
                state.links = [r for r in ddgs.text(q, region='ru-ru', max_results=15)]
                # 2. Картинки
                state.images = [r for r in ddgs.images(q, region='ru-ru', max_results=15)]
                # 3. Видео
                state.videos = [r for r in ddgs.videos(q, region='ru-ru', max_results=10)]
            
            if state.links:
                ctx = "\n".join([l.get('body','') for l in state.links[:3]])
                ans = get_ai_res([{"role":"system","content":"Ты Кусица. Отвечай максимально подробно."}, {"role":"user","content":f"Вопрос: {q}\nИнфо: {ctx}"}])
                state.ai_history = [{"content": ans}]
            
            state.last_q = q
        except Exception as e:
            st.error(f"Ошибка бесплатного поиска: {e}. Попробуйте через 5 секунд.")
...
with col_l:
    if st.button("КУСИЦА", key="logo_btn"):
        for k in keys_to_init: state[k] = [] if 's' in k or 'history' in k or 'extra' in k else None
        st.query_params.clear(); st.rerun()

with col_i:
    st.markdown(f'<div class="informer-box"><div class="informer-pill">📅 {d_s}</div><div class="informer-pill">🕒 {t_s} МСК</div><div class="informer-pill">🌡️ {w_m}</div><div class="currency-red">USD {u_v}₽</div><div class="currency-red">EUR {e_v}₽</div></div>', unsafe_allow_html=True)
...
if state.links:
    ...
    with t1:
        ...
        for l in state.links:
            domain = urlparse(l['href']).netloc
            fav = f"https://www.google.com/s2/favicons?sz=64&domain_url={domain}"
            st.markdown(f'<div style="margin-bottom:25px;"><span style="color:green; font-size:13px;">{domain}</span><br><img src="{fav}" style="width:18px; vertical-align:middle; margin-right:8px;"><a href="{l["href"]}" target="_blank" style="font-size:20px; color:#1a0dab; text-decoration:none;">{l["title"]}</a><div style="font-size:14px; color:#444; margin-top:5px;">{l.get("body","")}</div></div>', unsafe_allow_html=True)
    ...
    with t3:
        for v in state.videos:
            st.markdown(f'<div class="video-row"><img src="{v.get("image","https://via.placeholder.com/100")}" class="video-thumb"><div><div style="font-size:16px; font-weight:bold;">{v["title"]}</div><div style="color:green; font-size:12px;">{v.get("source","Видео")}</div><a href="{v["content"]}" target="_blank" style="color:#1a0dab; font-weight:bold;">▶️ Смотреть</a></div></div>', unsafe_allow_html=True)
    ...
    try:
        with DDGS() as ddgs:
            news = [r for r in ddgs.news("Россия сегодня", region='ru-ru', max_results=6)]
            for n in news:
