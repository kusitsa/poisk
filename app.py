...
# 4. ФУНКЦИИ API (ИСПРАВЛЕНО ГЕТТЕРЫ ДАННЫХ)
@st.cache_data(ttl=300) # Кэш на 5 минут
def get_header_data():
    # ДАТА И ВРЕМЯ (Работает всегда)
    tz_m = pytz.timezone('Europe/Moscow')
    now_m = datetime.now(tz_m)
    date_s = now_m.strftime("%d.%m")
    time_s = now_m.strftime("%H:%M")
    
    # ПОГОДА (Запрос к wttr.in)
    try:
        w_res = requests.get("https://wttr.in/Moscow?format=%t", timeout=2).text.strip()
        if "+" not in w_res and "-" not in w_res: w_res = "Облачно"
    except:
        w_res = "МСК"

    # ВАЛЮТА (Стабильный API)
    try:
        c_res = requests.get("https://open.er-api.com/v6/latest/USD", timeout=2).json()
        usd = round(c_res['rates']['RUB'], 1)
        eur = round(usd / (c_res['rates']['EUR'] / c_res['rates']['USD']), 1)
    except:
        usd, eur = "??", "??"
        
    return date_s, time_s, w_res, usd, eur
...
with col_i:
    ...
    <div class="informer-box">
        ...
        <div class="informer-pill">🌡️ {w_m}</div>
...
if state.links:
    ...
    with t1:
        ...
        for l in state.links:
            ...
            st.markdown(f'<div style="margin-bottom:25px; padding:10px;"><span style="color:green; font-size:13px;">{dom}</span><br><img src="{fav}" style="width:18px; vertical-align:middle; margin-right:8px;"><a href="{l["link"]}" target="_blank" style="font-size:20px; color:#1a0dab; text-decoration:none;">{l["title"]}</a><div style="font-size:14px; color:#444; margin-top:5px;">{l.get("snippet","")}</div></div>', unsafe_allow_html=True)
    ...
    with t2:
        if state.view_img_idx is not None:
            ...
            for i, img in enumerate(state.images):
                with cols[i % 2]:
                    st.markdown(f'<img src="{img["imageUrl"]}" style="width:100%; aspect-ratio:1/1; object-fit:cover; border-radius:15px; border:1px solid #eee;">', unsafe_allow_html=True)
    ...
    with t3:
        for v in state.videos:
            st.markdown(f'<div style="display:flex; gap:15px; background:#fff; padding:10px; border-radius:15px; border-bottom:1px solid #f0f0f0; align-items:center; margin-bottom:10px;"><img src="{v.get("imageUrl","https://via.placeholder.com/100")}" style="width:100px; height:100px; object-fit:cover; border-radius:12px;"><div><div style="font-size:16px; font-weight:bold;">{v["title"]}</div><div style="color:green; font-size:12px;">{v.get("source","Видео")}</div><a href="{v["link"]}" target="_blank" style="color:#1a0dab; font-size:14px; font-weight:bold;">▶️ Смотреть</a></div></div>', unsafe_allow_html=True)
