...
# 2. ДИЗАЙН (Яндекс-стайл + Мобильная версия)
st.markdown("""
    <style>
...
def get_ai_res(prompt_text):
    try:
        # Используем VseGPT (Российский прокси)
        api_key = st.secrets["VSEGPT_API_KEY"]
        url = "https://api.vsegpt.ru/v1/chat/completions"
        headers = {"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"}
        payload = {
            "model": "openai/gpt-4o-mini", # Самая быстрая и дешевая модель
            "messages": [{"role": "user", "content": f"Ответь коротко и ясно: {prompt_text}"}],
            "max_tokens": 500
        }
        res = requests.post(url, headers=headers, json=payload, timeout=20).json()
        return res['choices'][0]['message']['content']
    except Exception as e:
        return f"Кусица задумалась... Ошибка: {e}"
...
def check_conversion(query, usd_val, eur_val):
    q = query.lower()
    m_u = re.search(r'(\d+)\s*(доллар|usd|бакс)', q)
    if m_u: return f"{m_u.group(1)} USD", f"{round(int(m_u.group(1)) * usd_val, 2)} ₽"
    m_e = re.search(r'(\d+)\s*(евро|eur)', q)
    if m_e: return f"{m_e.group(1)} EUR", f"{round(int(m_e.group(1)) * eur_val, 2)} ₽"
    return None
    ...
    if st.session_state.get('last_q') != q:
        with st.spinner(" "):
            try:
                ...
                ans = get_ai_res(f"Вопрос: {q}. Данные: {links[:3]}")
...
if st.session_state.res:
    ...
    with tab1:
        ...
        if u_q: st.info(get_ai_res(f"Вопрос: {u_q}. Предыдущий ответ: {res['ans']}"))
        
        st.write("---")
        for l in res['links'][:st.session_state.limits['links']]:
            st.markdown(f'<div style="margin-bottom:20px;"><a class="search-title" href="{l["link"]}" target="_blank">{l["title"]}</a><br><small style="color:#006621;">{l["link"][:80]}...</small><div>{l.get("snippet","")}</div></div>', unsafe_allow_html=True)
