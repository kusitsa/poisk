...
st.markdown("""
    ...
    .stButton > button {
        ...
        background-color: #ffdb4d; color: black; border: none; font-size: 22px; font-weight: bold;
    }
    .alice-card { 
    ...
    if st.session_state.get('last_q') != q:
        with st.spinner(" "):
            try:
                ...
                ans = get_ai_answer([{"role":"system","content":"Ты Кусица. Отвечай кратко."}, {"role":"user","content":f"Вопрос: {q}\nДанные: {links[:3]}"}])
...
if st.session_state.res:
    ...
    with t1:
        ...
        for l in st.session_state.res['links'][:st.session_state.link_limit]:
            st.markdown(f'<div class="search-result"><a class="search-title" href="{l["link"]}" target="_blank">{l["title"]}</a><br><small style="color:#006621;">{l["link"][:70]}...</small><div class="search-desc">{l.get("snippet","")}</div></div>', unsafe_allow_html=True)
